...
export default function DbFilesPage({
  ...
  async function onUpload() {
    ...
    const res = await fetch(`/api/db/${encodeURIComponent(name)}/files`, {
      method: "POST",
      body: formData,
    });
  ...
  async function confirmDelete() {
    ...
    const res = await fetch(
      `/api/db/${encodeURIComponent(name)}/files/${encodeURIComponent(deleteConfirmation.fileId)}`,
      { method: "DELETE" }
    );
  ...
  return (
    <div className="px-6 py-8 max-w-6xl mx-auto w-full">
      <div className="mb-6">
        ...
        <h1 className="text-xl font-semibold mt-3">Files - <span className="font-mono">{name}</span></h1>
                  ...
                  <tr key={row.id} className="hover:bg-neutral-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-3">
                        <a
                          href={`/api/db/${encodeURIComponent(name)}/files/${encodeURIComponent(row.id)}`}
                          target="_blank"
                          rel="noreferrer"
