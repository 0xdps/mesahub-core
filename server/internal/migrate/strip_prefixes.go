...
//  3. control.db — databases.slug, buckets.slug, api_keys.scopes (JSON)
//  4. files/metadata.db — files.db_name ("bkt-B-<x>" → "bkt-<x>")
...
func (m *prefixMigration) run() error {
	...
	if err := m.updateControl(); err != nil {
		return fmt.Errorf("phase 3 (control.db): %w", err)
	}
	if err := m.updateFilesMetadata(); err != nil {
		return fmt.Errorf("phase 4 (files/metadata.db): %w", err)
	}
	log.Info().
...
func (m *prefixMigration) updateControl() error {
	ctrlPath := filepath.Join(m.dataPath, "control.db")
	if _, err := os.Stat(ctrlPath); os.IsNotExist(err) {
		return nil
	}

	db, err := openDB(ctrlPath)
	if err != nil {
		return err
	}
	defer db.Close()
	...
	if err := m.updateColumnPrefix(tx, "databases", "slug", "id", "D-"); err != nil {
		_ = tx.Rollback()
		return err
	}
	if err := m.updateColumnPrefix(tx, "buckets", "slug", "id", "B-"); err != nil {
		_ = tx.Rollback()
		return err
	}
	if err := m.updateAPIKeyScopes(tx); err != nil {
		_ = tx.Rollback()
		return err
	}
	return tx.Commit()
...
func (m *prefixMigration) updateAPIKeyScopes(tx *sql.Tx) error {
	rows, err := tx.Query(`SELECT id, scopes FROM api_keys WHERE scopes LIKE '%D-%' OR scopes LIKE '%B-%'`)
	if err != nil {
		return err
	}
	defer rows.Close()
	type row struct{ id, scopes string }
	var targets []row
	for rows.Next() {
		...
		targets = append(targets, row{id, scopes})
	}
	if err := rows.Err(); err != nil {
		return err
	}
	for _, t := range targets {
		newScopes, changed, err := rewriteScopes(t.scopes)
		if err != nil {
			log.Warn().Str("key_id", t.id).Err(err).Msg("migrate: could not parse api_key scopes — skipping")
			continue
		}
		if !changed {
			continue
		}
		if _, err := tx.Exec(`UPDATE api_keys SET scopes = ? WHERE id = ?`, newScopes, t.id); err != nil {
			return fmt.Errorf("update api_key scopes %s: %w", t.id, err)
		}
		m.updated++
	}
	return nil
...
func rewriteScopes(scopesJSON string) (string, bool, error) {
	var scopes []string
	if err := json.Unmarshal([]byte(scopesJSON), &scopes); err != nil {
		return scopesJSON, false, err
	}
	changed := false
	for i, s := range scopes {
		...
		rType, slug, level := parts[0], parts[1], parts[2]
		var newSlug string
		switch rType {
		case "db":
			newSlug, _ = stripPrefix(slug, "D-")
		case "bucket":
			newSlug, _ = stripPrefix(slug, "B-")
		default:
			continue
		}
		if newSlug != slug {
			scopes[i] = rType + ":" + newSlug + ":" + level
			changed = true
		}
	}
	if !changed {
		return scopesJSON, false, nil
	}
