...
// ── UUID route handlers (control mode) ───────────────────────────────────────

func (h *RestHandler) GetByUUID(w http.ResponseWriter, r *http.Request) {
	name, table, ok := h.resolveDBByUUID(w, r)
	if !ok {
		return
	}
	h.doGet(w, r, name, table)
...
func (h *RestHandler) PostByUUID(w http.ResponseWriter, r *http.Request) {
	name, table, ok := h.resolveDBByUUID(w, r)
	if !ok {
		return
	}
...
func (h *RestHandler) PatchByUUID(w http.ResponseWriter, r *http.Request) {
	name, table, ok := h.resolveDBByUUID(w, r)
	if !ok {
		return
	}
...
func (h *RestHandler) DeleteByUUID(w http.ResponseWriter, r *http.Request) {
	name, table, ok := h.resolveDBByUUID(w, r)
	if !ok {
		return
	}
...
func (h *RestHandler) resolveDBByUUID(w http.ResponseWriter, r *http.Request) (dbName, table string, ok bool) {
	...
	return templateName, table, true
}
