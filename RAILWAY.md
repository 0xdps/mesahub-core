...
- `ADMIN_TOKEN` environment variable shared with any service that connects to the file-db API

### Deployment Dependencies

- [file-db on GitHub](https://github.com/0xdps/file-db)
...
```bash
curl -X POST https://your-filedb.railway.app/api/db/mydb/query \
...
```bash
curl -X POST https://your-filedb.railway.app/api/db \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
...
By deploying file-db on Railway, you are one step closer to supporting a complete full-stack application with minimal burden. Host your servers, databases, AI agents, and more on Railway.
